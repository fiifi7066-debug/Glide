...
export default function PaymentModal({ isOpen, onClose, amount, onProcess }: PaymentModalProps) {
  ...
  const handlePay = async () => {
    ...
    setProcessing(false)
    setSuccess(true)
    setTimeout(() => { onProcess(method); setSuccess(false); onClose() }, 1500)
  }
      ...
      <div className="relative glass-card rounded-2xl max-w-md w-full p-6">
        ...
        {success ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 mt-1">GHS {amount.toFixed(2)} paid via {METHODS.find((m) => m.id === method)?.label}</p>
          </div>
        ) : (
          <>
            <div className="text-center mb-6">
              <h3 className="text-xl font-bold text-white">Complete Payment</h3>
              <div className="mt-3"><span className="text-4xl font-bold text-blue-400">GHS {amount.toFixed(2)}</span></div>
            </div>
              ...
              {METHODS.map((m) => (
                <button key={m.id} onClick={() => setMethod(m.id)}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left ${
                    method === m.id ? "border-blue-500/50 bg-blue-500/10" : "border-dark-border hover:border-gray-600"
                  }`}>
                  <span className="text-xl">{m.icon}</span>
                  <span className="text-sm font-medium text-gray-200">{m.label}</span>
                  {method === m.id && <div className="ml-auto w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center"><FiCheck size={12} className="text-white" /></div>}
                </button>
              ))}
            </div>

            <button onClick={handlePay} disabled={processing} className="w-full py-3.5 gradient-btn">
              {processing ? (
