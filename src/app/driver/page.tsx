...
export default function DriverDashboard() {
  const { user, activeTrip, setActiveTrip, activeDelivery, setActiveDelivery, pendingRequests, setPendingRequests, pendingDeliveries, setPendingDeliveries, driverLocation, setDriverLocation } = useStore()
  ...
  const fetchPendingRequests = useCallback(async () => {
    ...
    try { const res = await fetch("/api/trips/history"); if (res.ok) { const json = await res.json(); const all = json.data || []; setPendingRequests(all.filter((t: TripData) => t.status === "REQUESTED")) } } catch {}
    ...
    try { const res = await fetch("/api/deliveries/history"); if (res.ok) { const json = await res.json(); const all = json.data || []; setPendingDeliveries(all.filter((d: DeliveryData) => d.status === "REQUESTED")) } } catch {}
  }, [setActiveTrip, setActiveDelivery, setPendingRequests, setPendingDeliveries])

  useEffect(() => { fetchPendingRequests() }, [fetchPendingRequests])
  useEffect(() => { const interval = setInterval(fetchPendingRequests, 5000); return () => clearInterval(interval) }, [fetchPendingRequests])
  useEffect(() => {
    if (!showMap) return
    const interval = setInterval(() => { const lat = 5.6037 + (Math.random() - 0.5) * 0.02; const lng = -0.1870 + (Math.random() - 0.5) * 0.02; setDriverLocation({ lat, lng }) }, 3000)
    return () => clearInterval(interval)
  }, [showMap, setDriverLocation])

  const handleAcceptTrip = async (trip: TripData) => {
    try { const res = await fetch(`/api/trips/${trip.id}/accept`, { method: "PATCH" }); if (res.ok) { const json = await res.json(); setActiveTrip(json.data); setPendingRequests(useStore.getState().pendingRequests.filter((t) => t.id !== trip.id)) } } catch {}
  }

  const handleStartTrip = async () => {
    if (!activeTrip) return
    try { const res = await fetch(`/api/trips/${activeTrip.id}/start`, { method: "PATCH" }); if (res.ok) { const json = await res.json(); setActiveTrip(json.data) } } catch {}
  }

  const handleCompleteTrip = async () => {
    if (!activeTrip) return
    try { const res = await fetch(`/api/trips/${activeTrip.id}/complete`, { method: "PATCH" }); if (res.ok) { const json = await res.json(); setActiveTrip(json.data) } } catch {}
  }
  ...
  const isOnTrip = activeTrip && activeTrip.status !== "COMPLETED" && activeTrip.status !== "CANCELLED"
  const isOnDelivery = activeDelivery && activeDelivery.status !== "DELIVERED" && activeDelivery.status !== "CANCELLED"
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-white">Live Map</h3>
            <button onClick={() => setShowMap(!showMap)} className="text-sm text-blue-400 hover:underline">{showMap ? "Hide" : "Show"}</button>
          </div>
          {showMap && (
            <MapView pickupLat={activeTrip?.pickupLat || activeDelivery?.pickupLat} pickupLng={activeTrip?.pickupLng || activeDelivery?.pickupLng} dropoffLat={activeTrip?.dropoffLat || activeDelivery?.dropoffLat} dropoffLng={activeTrip?.dropoffLng || activeDelivery?.dropoffLng} driverLat={driverLocation?.lat} driverLng={driverLocation?.lng} height="400px" />
          )}
        </div>
      </div>
    </div>
