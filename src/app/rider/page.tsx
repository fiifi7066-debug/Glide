...
export default function RiderDashboard() {
  ...
  const fetchNearbyDrivers = useCallback(async () => {
    try {
      const res = await fetch("/api/drivers/nearby?lat=5.6037&lng=-0.1870")
      if (res.ok) {
  ...
  useEffect(() => {
    fetchNearbyDrivers()
    fetchActiveTrip()
  }, [fetchNearbyDrivers, fetchActiveTrip])
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          {!isTripActive && !searching && activeTrip === null && (
            <BookingForm onRequestRide={handleRequestRide} />
          )}
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 border border-primary text-primary font-medium rounded-xl hover:bg-primary/5 transition-colors text-sm"
                >
                  Rate Driver
                </button>
                <button
          ...
          {nearbyDrivers.length > 0 && !isTripActive && (
            <div>
              ...
              <div className="space-y-3">
                {nearbyDrivers.slice(0, 3).map((driver: NearbyDriver) => (
      ...
      <RatingModal
        ...
        targetName={activeTrip?.driver?.user?.name || "Driver"}
        targetRole="driver"
      />
